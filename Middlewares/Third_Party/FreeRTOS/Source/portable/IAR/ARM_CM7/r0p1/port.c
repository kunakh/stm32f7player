...
static void prvSetupMPU( void )
{
    ...
    extern uint32_t __ICFEDIT_region_QSPI_end__[];
    extern uint32_t __ICFEDIT_region_IRAM_start__[];
    extern uint32_t __ICFEDIT_region_IRAM_end__[];
    extern uint32_t __ICFEDIT_region_SDRAM_start__[];
    extern uint32_t __ICFEDIT_region_SDRAM_end__[];
	...
	if( *portMPU_TYPE == portEXPECTED_MPU_TYPE_VALUE )
	{
		...
		*portMPU_REGION_ATTRIBUTE =		( portMPU_REGION_READ_ONLY ) |
										...
										( prvGetMPURegionSizeSetting( ( uint32_t ) __ICFEDIT_region_QSPI_end__ - ( uint32_t ) __ICFEDIT_region_QSPI_start__ ) ) |
		...
		*portMPU_REGION_ATTRIBUTE =		( portMPU_REGION_READ_ONLY ) |
										( portMPU_REGION_CACHEABLE ) |
										( prvGetMPURegionSizeSetting( ( uint32_t ) __ICFEDIT_region_IROM_end__ - ( uint32_t ) __ICFEDIT_region_IROM_start__ ) ) |
										( portMPU_REGION_ENABLE );
...
void vPortStoreTaskMPUSettings( xMPU_SETTINGS *xMPUSettings, const struct xMEMORY_REGION * const xRegions, StackType_t *pxBottomOfStack, uint16_t usStackDepth )
{
	...
	if( xRegions == NULL )
	{
        ...
        xMPUSettings->xRegion[ 0 ].ulRegionBaseAddress =
				( ( uint32_t ) __ICFEDIT_region_SDRAM_start__ ) | /* Base address. */
				( portMPU_REGION_VALID ) |
				( portSTACK_REGION );
		...
		xMPUSettings->xRegion[ 1 ].ulRegionBaseAddress =
				( ( uint32_t ) __ICFEDIT_region_IRAM_start__ ) | /* Base address. */
				( portMPU_REGION_VALID ) |
				( portSTACK_REGION + 1 );
